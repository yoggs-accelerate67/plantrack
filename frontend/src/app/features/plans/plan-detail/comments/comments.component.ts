...
import { Comment } from '@shared/plan.model';
...
export class CommentsComponent implements OnInit {
  ...
  onCommentKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === 'ArrowDown') {
      ...
      this.selectedMentionIndex.set(
        Math.min(this.selectedMentionIndex() + 1, maxIndex),
      );
    } else if (event.key === 'ArrowUp') {
      event.preventDefault();
      this.selectedMentionIndex.set(
        Math.max(this.selectedMentionIndex() - 1, 0),
      );
    } else if (event.key === 'Enter' && !event.shiftKey) {
      event.preventDefault();
      const selectedUser =
        this.filteredMentionUsers()[this.selectedMentionIndex()];
      if (selectedUser) {
  ...
  submitComment(): void {
    if (
      ...
      this.newComment.content.trim().length === 0
    ) {
      return;
    }

    this.commentService
      .createComment(this.initiativeId, this.newComment)
      .subscribe({
        next: () => {
  ...
  saveEdit(comment: Comment): void {
    if (
      !this.editingCommentContent ||
      this.editingCommentContent.trim().length === 0
    ) {
      return;
    }
