...
import { Plan, PlanDetail, PageResponse } from '@shared/plan.model';
...
export class PlanService {
  ...
  getAllPlans(
    ...
    size: number = 10,
    sort: string = 'planId',
  ): Observable<PageResponse<Plan>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString())
      .set('sort', sort);
    return this.http.get<PageResponse<Plan>>(`${this.apiUrl}/plans`, {
      params,
    });
  }
  ...
  getPlansByUser(
    ...
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<PageResponse<Plan>>(
      `${this.apiUrl}/users/${userId}/plans`,
      { params },
    );
  }
  ...
  getPlansWithAssignedInitiatives(userId: number): Observable<Plan[]> {
    return this.http.get<Plan[]>(
      `${this.apiUrl}/users/${userId}/assigned-plans`,
    );
  }
  ...
  getCancelPreview(planId: number): Observable<CancelPreviewResponse> {
    return this.http.get<CancelPreviewResponse>(
      `${this.apiUrl}/plans/${planId}/cancel-preview`,
    );
  }
  ...
  cancelPlanWithCascade(planId: number): Observable<CancelResultResponse> {
    return this.http.post<CancelResultResponse>(
      `${this.apiUrl}/plans/${planId}/cancel`,
      {},
    );
  }
}
