...
import { Initiative, PageResponse } from '@shared/plan.model';
...
export class InitiativeService {
  ...
  createInitiative(
    ...
    return this.http.post<Initiative>(
      `${this.apiUrl}/milestones/${milestoneId}/initiatives`,
      initiative,
      { params },
    );
  }

  getInitiativesByMilestone(
    milestoneId: number,
    page: number = 0,
    size: number = 10,
  ): Observable<PageResponse<Initiative>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<PageResponse<Initiative>>(
      `${this.apiUrl}/milestones/${milestoneId}/initiatives`,
      { params },
    );
  }
  ...
  updateInitiative(
    ...
    return this.http.put<Initiative>(
      `${this.apiUrl}/initiatives/${initiativeId}`,
      initiative,
    );
  }
  ...
  getMyInitiatives(userId: number): Observable<Initiative[]> {
    return this.http.get<Initiative[]>(
      `${this.apiUrl}/users/${userId}/initiatives`,
    );
  }
}
