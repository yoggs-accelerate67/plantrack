    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
          ...
          <p class="text-slate-600">Welcome back! Here's an overview of your projects.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div class="card group hover:scale-105 transition-transform">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-slate-600 mb-2">Total Plans</p>
                <p class="text-4xl font-bold text-slate-900 mb-1">{{ stats()?.totalPlans || 0 }}</p>
          ...
          <div class="card group hover:scale-105 transition-transform">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-slate-600 mb-2">Active Initiatives</p>
                <p class="text-4xl font-bold text-slate-900 mb-1">{{ stats()?.activeInitiatives || 0 }}</p>
          ...
          <div class="card group hover:scale-105 transition-transform">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-slate-600 mb-2">Completed Milestones</p>
                <p class="text-4xl font-bold text-slate-900 mb-1">{{ stats()?.completedMilestones || 0 }}</p>
          ...
          <div class="card group hover:scale-105 transition-transform">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-4xl font-bold text-slate-900 mb-1">{{ stats()?.totalUsers || 0 }}</p>
        ...
        <div class="flex justify-between items-center mb-6">
          <div>
            <h2 class="text-2xl font-bold text-slate-900">Active Plans</h2>
            <p class="text-sm text-slate-600 mt-1">Manage and track your project plans</p>
          </div>
        ...
        @if (loadingService.isLoading()) {
          ...
          <div class="card p-16 text-center">
            ...
            <h3 class="text-xl font-bold text-slate-900 mb-2">No plans yet</h3>
            <p class="text-slate-600 mb-6 max-w-sm mx-auto">Get started by creating your first project plan. Organize milestones and track progress all in one place.</p>
            @if (authService.isManager()) {
              <button
                (click)="showCreateModal.set(true)"
            ...
            @for (plan of plans(); track plan.planId) {
              ...
              >
                <div class="flex justify-between items-start mb-4">
                  <h3 class="text-lg font-bold text-slate-900 group-hover:text-teal-600 transition-colors flex-1 pr-2">{{ plan.title }}</h3>
                  <span
                    class="px-3 py-1 rounded-full text-xs font-semibold flex-shrink-0"
                    [ngClass]="{
                      'bg-green-100 text-green-700 border border-green-200': plan.status === 'COMPLETED',
                      'bg-amber-100 text-amber-700 border border-amber-200': plan.status === 'IN_PROGRESS',
                      'bg-slate-100 text-slate-700 border border-slate-200': plan.status === 'PLANNED'
                    }"
                  >
                    {{ plan.status }}
                  </span>
                </div>
                <p class="text-sm text-slate-600 mb-4 line-clamp-2 min-h-[2.5rem]">{{ plan.description || 'No description provided' }}</p>
                <div class="space-y-3 pt-4 border-t border-slate-100">
                  <div class="flex justify-between items-center text-xs">
                    <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded font-medium">Priority: {{ plan.priority }}</span>
                    <span class="text-slate-500">{{ plan.userName }}</span>
                  </div>
                  <div>
        ...
        @if (showCreateModal()) {
          <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
            <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 border border-slate-200 animate-in zoom-in-95">
              <div class="flex items-center space-x-3 mb-6">
                ...
                <h3 class="text-2xl font-bold text-slate-900">Create New Plan</h3>
              </div>
